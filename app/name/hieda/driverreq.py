...
async def create_recruitment(
    ...
    # 1. 出発地・目的地の座標を特定（ジオコーディング）
    dep_lat, dep_lon = get_coordinates(req.departure)
    arr_lat, arr_lon = get_coordinates(req.destination)

    if dep_lat is None or arr_lat is None:
        raise HTTPException(status_code=400, detail="住所から座標を特定できませんでした。")

    # 2. 実際の走行ルート（道路沿い）を探索
    path_points, travel_duration = get_actual_route(dep_lat, dep_lon, arr_lat, arr_lon)
