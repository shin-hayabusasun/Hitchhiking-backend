...
def get_coordinates(address: str):
    if not address or not address.strip():
        return None, None
    geocoder = Nominatim(user_agent="driver_search_final_v1", timeout=5)
    try:
        location = geocoder.geocode(f"{address}, Japan")
        if location:
            return location.latitude, location.longitude
...
def get_min_distance_to_path(point_p, path_points):
    """
    点P(lat, lon)と経路点群との最小距離(の2乗)と、そのインデックスを返す
    """
    min_dist_sq = float('inf')
    closest_index = -1
...
@router.post("/search", response_model=SearchResponse)
async def search_passengers(req: SearchRequest, request: Request, db: Session = Depends(get_db)):
    logger.info("=== 運転者検索開始 (距離判定 + 時間補正版) ===")

    # 1. Auth
    session_id = request.cookies.get("session_id")
    # 開発用: セッション切れでも動くように0をセット（本番ではエラーにする）
    driver_id = 0
    if session_id:
        uid = get_current_user(session_id=session_id, db=db)
        if uid != "no":
            driver_id = int(uid)
    ...
    # 5. Filtering
    # 基本: 同乗者募集(type=1) & 募集中(status=0) & 現在時刻より後
    query = query.filter(
        modelDB.Recruitment.type == 1,
        modelDB.Recruitment.status == 0,
        modelDB.Route.dep_time >= datetime.now()
    ...
    # ★修正ポイント: 日付のタイムゾーンズレ対策
    if f.date:
        try:
            # 入力された日付 (JSTの00:00)
            target_date = datetime.strptime(f.date, '%Y-%m-%d')
            
            # UTCでの検索範囲を作成 (前後9-12時間広げる)
            # JST 00:00 は UTC 前日15:00
            ...
            logger.info(f"日付検索範囲(UTC考慮): {search_start} ~ {search_end}")
        except ValueError:
            pass
    ...
    if f.timeRange and (f.timeRange.start != "00:00" or f.timeRange.end != "23:59"):
        try:
            start_h = int(f.timeRange.start.split(":")[0])
            end_h = int(f.timeRange.end.split(":")[0])
            # 時間帯は日付を跨ぐと複雑になるため、単純にHourでフィルタ
            query = query.filter(extract('hour', modelDB.Route.dep_time).between(start_h, end_h))
        except:
            pass
    ...
    for r, v_dist in sql_results:
        ...
        # 経路データの読み込み (登録時に json.dumps しているので json.loads で戻す)
        try:
            path_points = json.loads(route_info.path_data) if route_info and route_info.path_data else []
        except:
            path_points = []
        ...
        # ★修正ポイント: 座標入力がある場合、ルートとの距離を厳密にチェック
        if p_start[0] is not None and p_end[0] is not None:
            # 最短距離(2乗)とインデックスを取得
            ...
            # スコア計算 (0.1度 ≒ 11km。係数500で計算)
            geo_score = 100 - (dist_val * 500)

            logger.info(f"ID:{r.recruitment_id} GeoScore:{geo_score:.1f} Order:{idx_start < idx_end}")

            # 1. 順序チェック
            if idx_start >= idx_end:
                continue # 逆方向なので除外

            # 2. 距離足切り (スコア0未満 ≒ ルートから合計20km以上離れている -> 除外)
            if geo_score < 0:
                continue # 遠すぎるので除外
            ...
            # 座標指定なし（日付検索のみなど）の場合は、ベクトルの相性などをスコアにする
